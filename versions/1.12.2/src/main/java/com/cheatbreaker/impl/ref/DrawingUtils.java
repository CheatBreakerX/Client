...
public class DrawingUtils implements IDrawingUtils {
    ...
    private void drawPanorama(float speed, int panoramaTimer, ResourceLocationBridge[] images) {
        ...
        for (int j = 0; j < i * i; ++j) {
            ...
            for (int k = 0; k < 6; ++k) {
                ...
                int l = 255 / (j + 1);
                buffer.pos(-1.0D, -1.0D, 1.0D).tex(0.0D, 0.0D).color(255, 255, 255, l).endVertex();
                buffer.pos(1.0D, -1.0D, 1.0D).tex(1.0D, 0.0D).color(255, 255, 255, l).endVertex();
                buffer.pos(1.0D, 1.0D, 1.0D).tex(1.0D, 1.0D).color(255, 255, 255, l).endVertex();
                buffer.pos(-1.0D, 1.0D, 1.0D).tex(0.0D, 1.0D).color(255, 255, 255, l).endVertex();
                tessellator.draw();
    ...
    private void rotateAndBlurSkybox(int scaledWidth, int scaledHeight, ResourceLocationBridge backgroundTexture) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            buffer.pos(scaledWidth, scaledHeight, 0).tex(0.0F + f1, 1.0D).color(1.0F, 1.0F, 1.0F, f).endVertex();
            buffer.pos(scaledWidth, 0.0D, 0).tex(1.0F + f1, 1.0D).color(1.0F, 1.0F, 1.0F, f).endVertex();
            buffer.pos(0.0D, 0.0D, 0).tex(1.0F + f1, 0.0D).color(1.0F, 1.0F, 1.0F, f).endVertex();
            buffer.pos(0.0D, scaledHeight, 0).tex(0.0F + f1, 0.0D).color(1.0F, 1.0F, 1.0F, f).endVertex();
        }
    ...
    public void renderSkybox(ResourceLocationBridge[] images, float speed, int scaledWidth, int scaledHeight, int panoramaTimer, ResourceLocationBridge backgroundLocation) {
        ...
        BufferBuilder buffer = tessellator.getBuffer();
        buffer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        buffer.pos(0.0D, scaledHeight, 0).tex(0.5F - f1, 0.5F + f2).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        buffer.pos(scaledWidth, scaledHeight, 0).tex(0.5F - f1, 0.5F - f2).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        buffer.pos(scaledWidth, 0.0D, 0).tex(0.5F + f1, 0.5F - f2).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        buffer.pos(0.0D, 0.0D, 0).tex(0.5F + f1, 0.5F + f2).color(1.0F, 1.0F, 1.0F, 1.0F).endVertex();
        tessellator.draw();
    }
}
