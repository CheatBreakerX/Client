...
public class VoiceChatGui extends AbstractGui {
    ...
    public void initGui() {
        ...
        if (cheatBreaker.getNetHandler().voiceChatEnabled && cheatBreaker.getNetHandler().getVoiceChannels() != null) {
            ...
            boolean bl = cheatBreaker.getNetHandler().getUuidList().contains(this.wrapped$mc.bridge$getThePlayer().bridge$getGameProfile().getId());
...
//        this.mc.entityRenderer.unloadSounds();
    ...
    protected void onMouseClicked(float f, float f2, int n) {
        ...
        if (this.voiceChannel != null) {
            ...
            if (this.undeafenButton.isMouseInside(f, f2)) {
                boolean bl;
                UUID iterator = this.wrapped$mc.bridge$getThePlayer().bridge$getGameProfile().getId();
                Ref.getMinecraft().bridge$getSoundHandler().bridge$playSound(Ref.getInstanceCreator().createSoundFromPSR(Ref.getInstanceCreator().createResourceLocation("gui.button.press"), 1.0f));
                ...
                this.undeafenButton.lIIIIlIIllIIlIIlIIIlIIllI((bl = cheatBreaker.getNetHandler().getUuidList().contains(this.wrapped$mc.bridge$getThePlayer().bridge$getGameProfile().getId())) ? "Un-deafen" : "Deafen");
            }
    ...
    private void lIIIIlIIllIIlIIlIIIlIIllI(float f, float f2, float f3, float f4) {
        ...
        for (VoiceUser voiceUser3 : arrayList) {
            ...
            f8 = f3 + (float)10;
            if (!voiceUser3.getUUID().equals(this.wrapped$mc.bridge$getThePlayer().bridge$getUniqueID())) {
                if (bl2) {
    ...
    private void lIIIIIIIIIlIllIIllIlIIlIl(float f, float f2, float f3, float f4) {
        ...
        for (VoiceUser voiceUser : this.voiceChannel.getUsers()) {
            ...
            if (!voiceUser.getUUID().equals(this.wrapped$mc.bridge$getThePlayer().bridge$getUniqueID()) && bl) {
                Ref.getMinecraft().bridge$getSoundHandler().bridge$playSound(Ref.getInstanceCreator().createSoundFromPSR(Ref.getInstanceCreator().createResourceLocation("gui.button.press"), 1.0f));
    ...
    public void keyTyped(char c, int n) {
        ...
        if (n == 25) {
            this.wrapped$mc.bridge$displayGuiScreen(null);
            this.wrapped$mc.bridge$setIngameFocus();
        }
    }
